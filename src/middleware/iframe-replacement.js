...
module.exports = function(req, res, next) {
    ...
    res.merge = function(view, model, callback) {
        if (!model.sourceUrl) {
            ...
            const sourcePlaceholder = model.sourcePlaceholder || 'body';

            let onError = model.onError;
            // resolve the template, either url to jquery object or html
            if (typeof(onError) === 'function') {
                onError = onError(req, model);
            }
            resolveTemplate(model.sourceUrl, onError).then(($template) => {
 ...
 */
function resolveTemplate(sourceUrl, onError) {
    return new Promise((resolve, reject) => {
        ...
        if (isUrl(sourceUrl) && cache.get(sourceUrl)) {
            ...
            return request(params).then((html) => {
                ...
                .catch((err) => {
                // request failed
                    reject(onError || `<div><h1>Why did you give me this item?</h1><h2>I cannot retrieve it: ${sourceUrl}</h2>For sure this is NOT similar!<div>`);
                });
        } else {
